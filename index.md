...
- [`cohort_define()`](https://tgerke.github.io/ggconsort/reference/cohort_define.md)
  constructs cohorts that are variations of the source data or other
  cohorts
- [`cohort_label()`](https://tgerke.github.io/ggconsort/reference/cohort_label.md)
  adds labels to each named cohort within the `ggconsort_cohort` object
...
- [`consort_box_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md)
  adds a text box to the CONSORT diagram, at a `row`/`col` grid position
  (laid out automatically at draw time) or at explicit coordinates
- [`consort_arrow_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md)
  adds an arrow to the CONSORT diagram
- [`consort_line_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md)
  adds a line (without an arrow head) the CONSORT diagram
...
control (the arms below sit at columns -1 and 1). A box named after a
cohort labels itself with that cohort’s label and count, so most boxes
need nothing but a name and a position, and
[`cohort_count_bullets()`](https://tgerke.github.io/ggconsort/reference/cohort_count.md)
builds the bulleted exclusion box in one call. Arrows just connect boxes
by name — vertically within a column, horizontally within a row, and
`end = c("treatment_a", "treatment_b")` draws the T-split into the study
arms. Stage badges land in the left margin, as in the official CONSORT
template. ggconsort measures every box when the plot is drawn and spaces
the grid to fit the device, so there are no coordinates to fiddle with
...
![CONSORT diagram: 1,200 patients assessed for eligibility, 262 excluded
(declined, prior chemotherapy, or bone metastasis), 938 randomized and
allocated 469 each to arms A and B, with Enrollment and Allocation stage
labels in the left
margin](reference/figures/README-example-consort-1.png)

The [complete CONSORT
article](https://tgerke.github.io/ggconsort/articles/consort.html)
continues this diagram through the Follow-up and Analysis stages, and
the [PRISMA
article](https://tgerke.github.io/ggconsort/articles/prisma.html) builds
a PRISMA 2020 flow diagram with the same verbs. Styling lives in
[`geom_consort()`](https://tgerke.github.io/ggconsort/reference/geom_consort.md)
— fills, borders, corner radius, fonts, and spacing, with per-box
overrides in
[`consort_box_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md).
Prefer to place things by hand?
[`consort_box_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md)
and
[`consort_arrow_add()`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md)
also accept explicit coordinates — see
[`?consort_box_add`](https://tgerke.github.io/ggconsort/reference/consort_box_add.md).
