...
need nothing but a name and a position, and `cohort_count_bullets()`
...
<img src="man/figures/README-example-consort-1.png" alt="CONSORT diagram: 1,200 patients assessed for eligibility, 262 excluded (declined, prior chemotherapy, or bone metastasis), 938 randomized and allocated 469 each to arms A and B, with Enrollment and Allocation stage labels in the left margin" width="100%" />
...
`geom_consort()` — fills, borders, corner radius, fonts, and spacing,
with per-box overrides in `consort_box_add()`. Prefer to place things by
hand? `consort_box_add()` and `consort_arrow_add()` also accept explicit
coordinates — see `?consort_box_add`.
